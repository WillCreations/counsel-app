...
function generateUserId(PDO $conn, string $prefix = 'USR'): string {
    ...
    if($error == false){
        ...
        var_dump( "dfname:$fullname", "duname:$username", "demail:$email", "dphone:$phone", "daddress:$address", "dgender:$gender", "ddob:$dob", "dgrade:$grade", "dsubject:$subject", "dscore:$score", "dcareer:$career_asp", "dphoto:$profilePicPath", "dpass:$pass");
        var_dump("hashedPass", $hashedPass);

        $query = 'INSERT INTO counselor (userid, dfname, duname, demail, dphone, daddress,  dphoto, dpass) VALUES (:userid, :dfname, :duname, :demail, :dphone, :daddress, :dphoto, :dpass);';
